...
contract FaucetTest is BaseSetup {
    ...
    function test_drip() public {
        ...
        uint256 _devBal = token.balanceOf(dev);
        ...
        uint256 _balAfterDrip = token.balanceOf(dev);

        /// The difference should be equal to the amount requested from the faucet
        assertEq(_balAfterDrip - _devBal, amountToDrip);

        // Test that the amount has been deducted from faucet
    ...
    function test_drip_fuzzy(address _recipient, uint256 _amount) public {
    ...
    /// the limit should be updated assertEq(faucet.limit(), 1000); }
